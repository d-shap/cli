 ...
 */
public abstract class AbstractYesNoCancelCommand extends AbstractMenuCommand {
    ...
    private final ValueHolder<String> _cancelMenuItemSymbol;

    private final ValueHolder<Lines> _cancelMenuItemLabel;

    /**
     * Create new object.
     */
    protected AbstractYesNoCancelCommand() {
        this(null);
    }
    ...
    /**
     * Get the symbol to select the "cancel" menu item.
     *
     * @return the symbol to select the "cancel" menu item.
     */
    protected abstract String getCancelMenuItemSymbol();

    /**
     * Get the label of the "cancel" menu item.
     *
     * @return the label of the "cancel" menu item.
     */
    protected abstract Lines getCancelMenuItemLabel();
    ...
    protected final List<Option> getOptions() {
        ...
        String cancelMenuItemSymbol = _cancelMenuItemSymbol.getValue();
        Lines cancelMenuItemLabel = _cancelMenuItemLabel.getValue();
        Option cancelMenuItem = new MenuItem(cancelMenuItemSymbol, cancelMenuItemLabel);
        options.add(cancelMenuItem);
    ...
    /**
     * Value loader for the "cancel" menu item symbol.
     *
     * @author Dmitry Shapovalov
     */
    private final class CancelMenuItemSymbolLoader implements ValueLoader<String> {

        CancelMenuItemSymbolLoader() {
            super();
        }

        @Override
        public String loadValue() {
            return getCancelMenuItemSymbol();
        }

    }

    /**
     * Value loader for the "cancel" menu item label.
     *
     * @author Dmitry Shapovalov
     */
    private final class CancelMenuItemLabelLoader implements ValueLoader<Lines> {

        CancelMenuItemLabelLoader() {
            super();
        }

        @Override
        public Lines loadValue() {
            return getCancelMenuItemLabel();
        }
